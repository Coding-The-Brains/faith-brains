...
export default async function SurahPage({
  ...
  const pageNav =
    totalPages > 1 ? (
      <nav className="flex items-center justify-between rounded-lg border border-border px-4 py-2 text-sm">
        {page > 1 ? (
          <Link href={`/quran/${n}?page=${page - 1}`} className="text-accent hover:underline">
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <header className="mb-8 text-center">
        ...
        <h1 className="font-semibold tracking-tight mt-2 text-2xl text-text">
      ...
      {page === 1 && basmala && (
        <p lang="ar" className="mb-8 text-center text-2xl text-text/90">
          {basmala}
        </p>
      )}
      ...
      <nav className="mt-10 flex justify-between text-sm">
        {n > 1 ? (
          <Link href={`/quran/${n - 1}`} className="text-accent hover:underline">
            ← Surah {n - 1}
          </Link>
        ) : (
          <span />
